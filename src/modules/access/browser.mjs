...
export function connectAccess(context) {
  ...
  async function renderAccess({ isCurrent }) {
    ...
    $('content').querySelectorAll('[data-invite]').forEach(button => {
      button.onclick = async () => {
        button.disabled = true;
        try { const result = await api('access/' + encodeURIComponent(button.dataset.invite) + '/invite', { method: 'POST', body: '{}' }); message(result.message); }
        catch (cause) { message(cause.message, true); }
        finally { button.disabled = false; }
      };
    });
