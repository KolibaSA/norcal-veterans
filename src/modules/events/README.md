...
- Optional Volunteer, Donate and Buy Tickets actions use an enable flag plus an HTTPS-only URL. Every shared event card can render any or all three actions when its matching flag and URL are present. Their compact one-row treatment does not change the card height.
