...
- Optional `volunteer_enabled`/`volunteer_url` and `donate_enabled`/`donate_url` fields are HTTPS-only. Their actions render only on VFW Post 8151 Upcoming Events cards when both the matching flag and URL are present.
