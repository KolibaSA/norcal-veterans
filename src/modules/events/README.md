...
- `organizationUpcomingEvents(events, organizationId, now?)` renders published upcoming cards in date order with a simple empty state. All organization profiles use this view, including American Legion pages and both Detachment 627 URLs. The shared Events page keeps its calendar, and HQ editing is unchanged.
