...
- `organizationUpcomingEvents(events, organizationId, now?, organizations?)` renders published upcoming cards in date order with a prominent Pacific month/day header and a simple empty state. VFW Post 8151 community occurrences are grouped by normalized title, organization and location after past occurrences are removed; multiple future dates reuse one extended shared card and a lone remaining date returns to the standard single-date card. Its organization meetings and every other organization retain the standard profile cards. The shared Events page omits regular `Organization meeting` entries.
...
- `eventsPagePublic(url, events, organizations = [])` and `eventDetail(event, organizations = [])` return page HTML. The public index groups compact event cards by month, uses an organization-provided HTTPS image or a simple event-specific fallback graphic, and applies the host organization's visual theme to the card edge. Accepted invited organizations can be supplied through `accepted_organization_ids` and are the only partners listed on the card.
- Optional Volunteer, Donate and Buy Tickets actions use an enable flag plus an HTTPS-only URL. Enabled ticket actions appear on every event detail page; all three actions render on VFW Post 8151 Upcoming Events cards when their matching URL is present.
