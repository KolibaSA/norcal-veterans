...
function eventVisual(v){
 ...
 if(/poppy/.test(text))return {key:'poppy',image:'/assets/events/event-poppy.webp'};
 if(/army\s*[–—-]?\s*navy|football|watch party/.test(text))return {key:'football',image:'/assets/events/event-football.webp'};
 if(/volunteer|service day|community service|cleanup|clean-up/.test(text))return {key:'volunteer',image:'/assets/events/event-volunteer.webp'};
 if(/ceremon|memorial|remembrance|wreath|honou?r|commemor/.test(text))return {key:'ceremony',image:'/assets/events/event-ceremony.webp'};
 return {key:'community',image:'/assets/events/event-volunteer.webp'};
}
function eventCard(v){
 const listed=isVfw8151Meeting(v),visual=eventVisual(v);
 return `<article class="panel event-card event-card--${visual.key}">
  <div class="event-card-media"><img src="${visual.image}" alt="" width="1440" height="480" loading="lazy" decoding="async"><span class="event-card-county">${pe(v.county)} County</span></div>
  <div class="event-card-body">
   <span class="event-card-type">${icons[visual.key]}${pe(v.kind)}</span>
   <h2>${listed?pe(v.title):`<a href="/events/${pe(v.id)}">${pe(v.title)}</a>`}</h2>
   <div class="event-card-facts">
    <p class="event-card-fact">${icons.calendar}<span>${pe(publicEventDate(v))}</span></p>
    <p class="event-card-fact">${icons.location}<span>${pe(v.venue)}</span></p>
   </div>
   <p class="event-card-description">${pe(v.description)}</p>
   ${listed?'<span class="small-note">Listed meeting — confirm details with the post.</span>':`<a class="event-card-link" href="/events/${pe(v.id)}">Event details &amp; attendance <span aria-hidden="true">→</span></a>`}
  </div>
 </article>`;
}
