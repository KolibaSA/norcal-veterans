...
function loadScript(src, attributes = {}) {
  return new Promise((resolve, reject) => {
    ...
    script.onload = resolve; script.onerror = () => reject(new Error('Secure sign-in could not load. Please refresh and try again.'));
...
export async function startClerkPage() {
  const message = document.getElementById('auth-message');
  try {
    const clerk = await loadClerk();
    if (!clerk) throw new Error('Organization sign-in is being configured.');
    if (location.pathname.startsWith('/hq/sign-out')) { await clerk.signOut({ redirectUrl: '/hq/sign-in' }); return; }
    if (clerk.session?.status === 'active') { location.replace('/hq'); return; }
