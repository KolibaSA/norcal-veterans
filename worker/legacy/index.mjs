...
import { ClerkResponse, withClerkHeaders } from '../../src/shared/clerk-auth.mjs';
import { clerkPublicRoute, clerkCSP } from '../../src/shared/clerk-pages.mjs';
...
async function headquartersRequest(req, env, responseContext) {
    ...
    try {
      ...
      const signInRoute = clerkPublicRoute(req, env);
      if (signInRoute) return signInRoute;
      if (!(path === '/hq' || path.startsWith('/hq/') || path.startsWith('/api/hq/'))) return fail('Not found.', 404);

      operation = 'authentication';
      let user;
      try { user = await verifyIdentity(req, env, responseContext); }
      catch (error) {
        if (error instanceof ClerkResponse) return error.response;
        if (error instanceof HTTPError) throw error;
        if (env.HQ_AUTH_PROVIDER === 'clerk' && error.message !== 'AUTH_NOT_CONFIGURED' && path.startsWith('/hq') && ['GET', 'HEAD'].includes(req.method)) {
          return new Response(null, { status: 302, headers: { Location: '/hq/sign-in', 'Cache-Control': 'private, no-store' } });
        }
        return fail(error.message === 'AUTH_NOT_CONFIGURED' ? 'Headquarters sign-in is being configured. Private records are locked.' :
          'Sign in to headquarters to continue.', error.message === 'AUTH_NOT_CONFIGURED' ? 503 : 401);
      }
      ...
      if ((path === '/hq' || path === '/hq/') && ['GET', 'HEAD'].includes(req.method)) {
        const html = env.HQ_AUTH_PROVIDER === 'clerk' ? headquarters.replace('<body>', '<body data-auth-provider="clerk">').replace('/cdn-cgi/access/logout', '/hq/sign-out') : headquarters;
        return new Response(req.method === 'HEAD' ? null : html, { headers: {
          'Content-Type': 'text/html;charset=utf-8', 'Cache-Control': 'private, no-store',
          'Content-Security-Policy': env.HQ_AUTH_PROVIDER === 'clerk' ? clerkCSP(env, scriptHash) : `default-src 'self'; script-src 'self' '${scriptHash}'; style-src 'self' 'unsafe-inline'; img-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'; form-action 'self'`,
          'X-Content-Type-Options': 'nosniff', 'Referrer-Policy': 'same-origin'
        } });
      }
      if (!['GET', 'HEAD'].includes(req.method) && req.headers.get('Origin') !== url.origin) return fail('Request origin rejected.', 403);
      operation = 'headquarters';
      if (path === '/api/hq/me' && req.method === 'GET') return json({ ...user, grants, uploads: false, invitationsEnabled: env.HQ_AUTH_PROVIDER === 'clerk', processorConnected: env.HQ_REQUEST_AGENT_ENABLED === 'true' });
...
export default {
  async fetch(req, env) {
    ...
    return withClerkHeaders(await headquartersRequest(req, env, responseContext), responseContext.headers);
  }
};
