...
export default {async fetch(req,env){const url=new URL(req.url),path=url.pathname;
 ...
 if(path==='/api/hq/me')return json({...user,grants,uploads:!!env.FILES,processorConnected:true});
 ...
 if(!permitted(user,grants,{region_id:region,organization_id:org},x.status==='published'?'publish':'write'))return fail('This record is outside your assignment.',403);
 const id=crypto.randomUUID(),now=new Date().toISOString(),mutation=crypto.randomUUID();await env.DB.batch([stmt(env,'INSERT INTO records(id,kind,title,body,region_id,organization_id,status,payload,created_by,created_at,updated_at,mutation_id) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)',id,kind,title,clean(x.body,20000),region,kind==='organization'?id:org,x.status,JSON.stringify(x.payload||{}),user.email,now,now,mutation),audit(env,user,'create',id,mutation)]);return json({id},201)
 }
