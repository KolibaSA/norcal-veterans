...
test('Detachment 627 has a complete organization-owned web experience',()=>{
 for(const path of ['/mcl-yolo','/organizations/mcl-yolo']){
  ...
  assert.ok(page.html.includes('/organization?org=mcl-yolo'));
  assert.ok(page.html.includes('<link rel="canonical" href="https://yolo-county-veterans.smartzgraphics.workers.dev/mcl-yolo">'));
...
test('all deliverable routes/assets respond with expected content and headers',async()=>{
 for(const url of ['/','/mcl-yolo','/about','/resources','/for-organizations','/data.json','/styles.css','/app.js','/og.png','/favicon.svg','/health']){const r=await worker.fetch(new Request('https://test'+url));assert.equal(r.status,200,url);}
