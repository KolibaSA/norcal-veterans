...
test('logo gallery keeps every organization reachable with accessible post and location labels',async()=>{
 ...
 test('VFW 8151 cards preserve meeting listings and organization isolation',()=>{
  ...
  assert.equal(page.status,200);assert.ok(page.html.includes('id="upcoming-events"'));assert.ok(!page.html.includes('id="calendar"'));assert.equal((page.html.match(/class="panel event-card vfw-meeting-card"/g)||[]).length,12);assert.ok(!page.html.includes('href="/events/vfw8151-meeting-'));
...
test('VFW, Legion and MCL directory cards share the branded card design with family themes',()=>{
 ...
 assert.ok(page.html.includes('/styles.css?v=why-norcal-images-20260921-1'));
...
test('every organization type uses the shared branded profile design and its family theme',()=>{
 ...
 for(const r of records){
  ...
  assert.ok(page.html.includes('/styles.css?v=why-norcal-images-20260921-1'),r.id);
...
test('homepage renders the responsive Why NorCalVeterans cards and contact pop-up',()=>{
 const page=render(new URL('https://test/yolo-solano'));
 assert.equal(page.status,200);
 ...
 const styles=readFileSync(new URL('../public/styles.css',import.meta.url),'utf8');
 for(const phrase of ['.regional-purpose-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr))','.regional-purpose-grid{grid-template-columns:1fr}','why-norcal-background.png','why-veterans-find.png','why-organizations-share.png','why-organizations-coordinate.png','.regional-contact-popover::backdrop'])assert.ok(styles.includes(phrase),phrase);
});
