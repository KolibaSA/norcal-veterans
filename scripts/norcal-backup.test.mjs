...
test('active HQ SQL restore verifies current schema and its revision baseline', () => {
  ...
  assert.deepEqual(result.upgrades_checked, ['0004_vfw_8151_meeting_plan.sql', '0005_super_admin.sql', '0006_clerk_identities.sql']);
});
...
test('additive restore rehearsal preserves existing data counts and creates one baseline per record', () => {
  ...
  assert.deepEqual(upgraded.upgrades_checked, ['0002_request_runs.sql', '0003_record_revisions.sql', '0004_vfw_8151_meeting_plan.sql', '0005_super_admin.sql', '0006_clerk_identities.sql']);
