...
test('annual meeting grid lists all twelve VFW 8151 monthly meeting boxes',()=>{
 ...
 assert.ok(html.includes('January'));assert.ok(html.includes('December'));assert.ok(html.includes('Christmas Potluck'));assert.ok(html.includes('7:00 PM'));assert.ok(!html.includes('Other post'));assert.ok(html.includes('All twelve monthly meeting slots are listed below.'));
});
