...
- Clerk authentication, sign-in pages and browser session lifecycle live in shared `clerk-*` modules, composed through `src/app/clerk-page.mjs` and the HQ entry. Invitation administration belongs to the Access module. See [migration status/contracts](docs/clerk-migration.md) before changing authentication; production cutover is not yet complete.
